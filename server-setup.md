...
Fabulously Forgified is a strictly client-sided modpack, meaning it works and behaves mostly the same on **every server that allows vanilla clients**. 
...
If you still like Fabric a lot, you can [install it on a server as well](https://fabricmc.net/use/?page=server) and use [server-side optimization mods](https://modrinth.com/mods?f=categories!=cursed&f=categories:optimization&v=1.21.1&e=server), [including those in FF](https://github.com/Fabulously-Optimized/fabulously-optimized/blob/main/INCLUDED-MODS.md#smooth).
...
#### How your server works in Fabulously Forgified

The following icons show the status of the server. Icons are shown on the bottom right corner of the chat box.

* **1.18.2 or below (e.g. ViaVersion)**: FF users will see ![optional](https://i.ibb.co/hstcjW7/neutral.png).
    * No messages can be reported by anyone. On vanilla clients, gray bars are displayed on the left of the messages.
* **1.19 or up**:
    1. Using [one of the mods/plugins](#how-to-protect-your-users): FF users will see ![disabled](https://i.ibb.co/QDFzXCT/secure.png)
        * No messages can be reported by anyone. On vanilla clients, gray bars are displayed on the left of the messages.
    2. In other cases, FF users will see ![unknown](https://i.ibb.co/Yb1n6fW/unknown.png) until they send the first chat message.
        1. If `enforce-secure-profile` = `true` (default), FF users will see ![enabled](https://i.ibb.co/2YgMHpR/insecure.png).
            * All messages can be reported.
        2. If `enforce-secure-profile` = `false` (recommended), FF users will see ![optional](https://i.ibb.co/hstcjW7/neutral.png).
            * Vanilla users' messages can be reported and disabled FF users' messages can not. On vanilla clients, gray bars are displayed on the left of FF users' messages.
* **Realms**: FF users will see ![realms](https://i.ibb.co/gTxw84X/realms.png).
...
- **All 1.19+ backend servers**: set `enforce-secure-profile` to `false` in _server.properties_
  ...
  - If no other measures are taken alongside this:
      - FF users will see ![optional](https://i.ibb.co/hstcjW7/neutral.png) and their messages cannot be reported.
...
- **Paper/Purpur/Pufferfish**: install the [FreedomChat](https://modrinth.com/mod/freedomchat) plugin.
   - Ensure `rewrite-chat` and `send-prevents-chat-reports-to-client` are set to `true` inside the FreedomChat's config file to make FF users see a ![disabled](https://i.ibb.co/QDFzXCT/secure.png) icon near chat.
- **Fabric/Forge/NeoForge**: install [No Chat Reports](https://www.curseforge.com/minecraft/mc-mods/no-chat-reports)
   - FF users will get a ![disabled](https://i.ibb.co/QDFzXCT/secure.png) icon near chat.
...
- **LAN worlds**: host from Fabulously Forgified or install [No Chat Reports](https://www.curseforge.com/minecraft/mc-mods/no-chat-reports) on the client
  - FF users will get a ![disabled](https://i.ibb.co/QDFzXCT/secure.png) icon near chat.
- **LAN-like worlds** ([e4mc](https://e4mc.link/) - in FF, [World Host](https://modrinth.com/mod/world-host), [Essential](https://essential.gg/) etc): host from Fabulously Forgified or install [No Chat Reports](https://www.curseforge.com/minecraft/mc-mods/no-chat-reports) on the client
  - FF users will see ![optional](https://i.ibb.co/hstcjW7/neutral.png) and their messages cannot be reported.
...
Did you know that it is possible to have content mods (e.g. new blocks and items) on the server without requiring them to be installed on the client? That means that you can add new stuff to the server while your users use unmodified Fabulously Forgified!
...
This would disallow Fabulously Forgified, despite it having mods that do exactly the same thing.

> **DO**: "Performance and visual-enchancing mods are allowed."

This would allow Fabulously Forgified in your server without requiring further clarification.
...
This would already disqualify Fabulously Forgified, as it has more.
...
Fabulously Forgified does not use Forge.
